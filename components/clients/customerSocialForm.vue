        ...
        CommonCard(color="lightBlue" :loading="loading")
            ...
            template(#body)
                v-form(class="d-flex flex-column" ref="form")
                    v-row(v-for="(contact, index) in formData.social" :key="contact.type")
...
const addRow = (type) => {
    formData.social.push({ type: null, value: '', privacy: null })
}
...
const submitData = () => {
    ...
    updateClient(props.client._id, { social }).then(response => {
...
watch(() => props.isDialogVisible, () => {
    if (props.client) {
        const social = JSON.parse(JSON.stringify(props.client.social))
        formData.social = [
            ...social
        ]
    } else {
