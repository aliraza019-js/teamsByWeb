<template lang="pug">
  v-card(width="100%" flat :loading="loading")
    v-toolbar(:title="$t('admin.about')" flat color="transparent")
      .underline
  ...
  // methods
  const getData = async () => {
    const res = await myFetch('/pub/corporate/about', {method: 'GET'});
    htmlContent.value = res.content;
  }
  const saveData = async () => {
    loading.value = true;
    try {
      await myFetch('/v2/corporate/about', { method: 'POST', body: {content: htmlContent.value } })
    } catch (e) {
